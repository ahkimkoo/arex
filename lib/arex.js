...
const sentiment_zh = require('./sentiment.js');
...
const get_title = function(html){
	...
	if(h1_matched&&h1_matched.length>1&&title_matched && title_matched.length){
		var tmch = title_matched[1].replace(/(?:\n|\r)/gm,'');
		var hmch = h1_matched[1].replace(/(?:\n|\r)/gm,'');
		if(tmch && tmch.indexOf(hmch.substring(0,10))<0)title = tmch;
	}
 ...
 */
const get_article_sync = function(html, exptd_len=120, shingle=false, min=150, max=350, filter=[]){
	let body = html.replace(/^[\s\S]*(<body[\s\S]*?<\/body>)[\s\S]*$/img,'$1');
	body = filterNoneContentTags(body);
	let blocks = get_blocks(body);
	let block_score  = get_block_score(blocks,5);
	...
	let content　= article_content.join('\n');
	let summarizer = new Summarizer(shingle,filter);
	return {
		'content' : content　,
		'title' : title,
		'pubdate' : grate.length>0?get_pubdate_base_block(blocks, grate[0][0]):get_pubdate(html),
		'summary' : summarizer.summarize(content, exptd_len, title),
		'sentiment' : sentiment_zh(article_content.join('\n')).score
	}
}

const summarize = function(content, exptd_len=120, shingle=false, min=150, max=350, filter=[], title){
	...
	content　= blocks.join('\n');
	let summarizer = new Summarizer(shingle,filter);
	return summarizer.summarize(content, exptd_len, title);
}
...
exports.sentiment = function(content, inject){
	content = filterNoneContentTags(content);
	let blocks = get_blocks(content);
	content　= blocks.join('\n');
	return sentiment_zh(content, inject);
}
