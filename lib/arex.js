...
const get_title = function(html){
	...
	let matched = h1_regex.exec(html);
	if(matched&&matched.length>1)title = matched[1];
	else{
		let title_regex = new RegExp('<title[^>]*?>([^<]+)</title>','i');
		let matched1 = title_regex.exec(html);
		if(matched1&&matched1.length>1)title = matched1[1];
	}
