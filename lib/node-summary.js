...
class Summarizer{
	...
	summarize(content, expected_length, title){
	    ...
	    for(let i=0;i<textRank.length;i++){
	    	...
	    	let title_similarity = spaces_title ? stringSimilarity.compareTwoStrings(spaces_title, spaces_setence) : 0.5;
	    	copied_rank.push([textRank[i],isummary.rank(origin_setence),title_similarity,i]);
	    }
	    // copied_rank.sort((a,b)=>b[0]-a[0]);
	    // copied_rank.sort((a,b)=>sentences[b[3]].length/b[0]-sentences[a[3]].length/a[0]);
	    copied_rank.sort((a,b)=>b[2] * b[1] * b[0] - a[2] * a[1] * a[0]);
	    // copied_rank.sort((a,b)=>b[1]  - a[1]);

	    let selectIndex = [];
	    let summary_size = 0;
	    let forb_rules = [];
	    for(let i=0;i<copied_rank.length;i++){
	    	let ordinal = copied_rank[i][3];
