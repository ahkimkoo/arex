...
class Summarizer{
	...
	summarize(content, expected_length){
	    ...
	    for(let i=0;i<textRank.length;i++){
	    	copied_rank.push([textRank[i],isummary.rank(sentences[i]),i]);
	    }
	    // copied_rank.sort((a,b)=>b[0]-a[0]);
	    // copied_rank.sort((a,b)=>sentences[b[2]].length/b[0]-sentences[a[2]].length/a[0]);
