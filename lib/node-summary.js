...
class Summarizer{
	...
	getSimilarityGraph(sentences){
	    ...
	    for(let s in sentences) {
	      ...
	      for(let t in sentences) {
	        sentenceSimilarity.push(levenshtein.get(sentences[s], sentences[t]));
	      }
	...
	getSelectedIndex(textRank, min, max){

	}

	summarize(content, expected_length){
	    	...
	    	for(let y = 0;y<terms[x].length;y++){
	    		if(this.shingle){
		    		...
		    		for(let z=y;z<terms[x].length;z++){
		    			...
		    			if(z>y && cu_size>expected_length*1.5)break;
	    ...
	    for(let i=0;i<textRank.length;i++){
	    	copied_rank.push([textRank[i],i]);
	    }
	    copied_rank.sort((a,b)=>b[0]-a[0]);
	    let selectIndex = [];
	    let summary_size = 0;
	    for(let i=0;i<copied_rank.length;i++){
	    	summary_size += sentences[copied_rank[i][1]].length;
	    	if(selectIndex.length>0){
	    		if(summary_size>expected_length*1.5)break;
	    		else selectIndex.push(copied_rank[i][1]);
	    	}else selectIndex.push(copied_rank[i][1]);
	    }
	    selectIndex.sort();
	    let selectSentences = [];
...
var k = summarizer.summarize(content, 200);
console.dir(k);
*/