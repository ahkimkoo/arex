...
class Summarizer{
	...
	score(sentence_rank, model_score, title_similarity, len_score){
		return sentence_rank * 0.5 + model_score * 0.2 + title_similarity * 0.1 + len_score * 0.2;
	}
	...
	summarize(content, expected_length, title){
	    ...
	    for(let i=0;i<textRank.length;i++){
	    	...
	    	let title_similarity = spaces_title ? stringSimilarity.compareTwoStrings(spaces_title, spaces_setence) : 0.1;
	    ...
	    copied_rank.sort((a,b)=> b[0] - a[0]);
	    // copied_rank.sort((a,b)=>b[1] * b[0] - a[1] * a[0]);
	    // copied_rank.sort((a,b)=>b[0]  - a[0]);
